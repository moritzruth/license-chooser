<template>
  <div
    class="fixed top-0 left-0 w-100vw h-100vh p-4 md:p-10 flex items-center justify-center bg-gray-200 transition duration-200 overflow-y-hidden"
    :class="open ? 'bg-opacity-50' : 'bg-opacity-0 pointer-events-none'"
  >
    <div
      ref="dialogElement"
      role="dialog"
      :aria-modal="Boolean(open)"
      class="bg-white shadow-lg rounded-lg transition duration-200 transform max-w-900px max-h-full flex-grow overflow-y-auto"
      :class="open || 'scale-70 opacity-0'"
    >
      <article v-if="lazyLicense">
        <div class="sticky top-0 bg-white p-5 md:px-10 md:py-5 shadow-md">
          <div class="font-mono text-gray-500 text-lg">
            {{ lazyLicense.id }}
          </div>
        ...
        <div class="p-5 pt-8 md:p-10 md:pt-8">
          ...
          <BadgesBag badge-class="bg-blue-200 text-blue-600" :items="lazyLicense.permissions.map(item => permissionBadges[item])"/>
          <span v-if="lazyLicense.permissions.length === 0" class="text-gray-600 italic">None</span>
          <h3 class="font-bold text-lg mb-1 mt-2">Conditions</h3>
          <span v-if="lazyLicense.conditions.length === 0" class="text-gray-500 text-lg italic -mt-1 block">None</span>
          <BadgesBag v-else badge-class="bg-yellow-200 text-yellow-600" :items="lazyLicense.conditions.map(item => conditionBadges[item])"/>
          <h3 class="font-bold text-lg mb-1 mt-2">Limitations</h3>
          <span v-if="lazyLicense.limitations.length === 0">None</span>
      ...
      <div class="flex space-x-2 justify-end mt-5 sticky bottom-0 bg-white px-4 md:px-10 py-4 shadow-umd space-x-4">
        <button class="px-5 py-2 bg-blue-400 rounded-md font-bold text-white focus:outline-none focus-visible:ring-4 ring-blue-400 ring-offset-2 w-45" @click="copy()">
          {{ notShowCopied ? "Copy to clipboard" : "Copied!" }}
        </button>
        <button class="close-button px-5 py-2 bg-gray-200 rounded-md focus:outline-none focus-visible:ring-4 ring-gray-300 ring-offset-4" @click="close()">
          Close
        </button>
      </div>
    </div>
