<template>
  <div class="bg-white dark:bg-gray-800 rounded-md shadow-md p-5 pb-4 group relative">
    ...
    <div class="font-bold mb-2 pr-12">
      <span class="font-mono mr-1 text-gray-500 dark:text-gray-400">{{ license.id }}</span>
      {{ license.title }}
    </div>
    <p class="line-clamp-2 mb-3">
      {{ license.description }}
    </p>
    <BadgesBag badge-class="bg-blue-200 text-blue-800" :items="license.permissions.map(item => permissionBadges[item])"/>
    <BadgesBag badge-class="bg-yellow-200 text-yellow-900" :items="license.conditions.map(item => conditionBadges[item])"/>
    <BadgesBag badge-class="bg-red-100 text-red-600" :items="license.limitations.map(item => limitationBadges[item])"/>
    <div class="transition-all duration-200 flex justify-end sm:absolute right-5 bottom-4">
      <button
        class="py-2 px-4 mt-2 bg-gray-200 dark:bg-gray-700 rounded-md -md:w-full
               focus:outline-none focus-visible:ring-2 ring-offset-4 ring-offset-gray-50 dark:ring-offset-gray-800 ring-blue-400"
        @click="openModal()"
      >
        More information
      </button>
